  ...
  return (req, res) => {
    Model.getAll(req.query, (err, data) => {
      if (err)
        res.status(500).send({
          message:
            err.message || "Error intentando recuperar los datos.",
          error: err
        })
      else res.send(data)
    }, model)
    ...
    Model.remove(req.params.id, (err, data) => {
      if (err) {
        if (err.kind === "not_found") {
          ...
          res.status(500).send({
            message: "No se pudo borrar el registro con id id " + req.params.id,
            error: err
          })
        }
      } else res.send({ message: `Los registros fueron eliminados con éxito!` })
    }, model)
  }
}
